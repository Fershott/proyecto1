...
> 💡 ¿Prefieres automatizar los pasos? Ejecuta `./tools/update_branch.py --branch main --sync-branch tu-rama` (o `python -m tools.update_branch ...`) para que el script descargue, actualice y regrese a tu rama de trabajo de forma asistida. Si tienes cambios locales que quieres conservar sin confirmarlos, añade `--auto-stash` y la herramienta los guardará y restaurará automáticamente.
...
2. Revisa el estado de tu copia con:
   ...
   git status -sb
   ```
   Si tienes cambios sin comprometer, decide si deseas confirmarlos (`git commit`) o crear un _stash_ temporal (`git stash`). El asistente también puede hacerlo por ti con `./tools/update_branch.py --auto-stash ...`.
