...
|-----------|----------------|-----------|
| Organización académica centrada en tareas y recordatorios | La aplicación se divide en pestañas "Acceso", "Tareas", "Pomodoro", "Calendario", "Recordatorios" y "Resúmenes", cargando datos reales desde la API y permitiendo marcar tareas como completadas. | `App.jsx` define las pestañas y conecta los endpoints de tareas y recordatorios. |
...
| Recordatorios notificables | `ReminderList` permite crear y editar avisos, actualiza la hora desde un editor en línea y genera correos personalizados ligados al proveedor activo. | `ReminderList.jsx` renderiza el formulario de edición y `main.update_reminder` envía el correo mediante `send_reminder_email`. |
