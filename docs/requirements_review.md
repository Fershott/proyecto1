...
| Inicio de sesión ligado a correos compatibles con notificaciones | La pantalla de acceso solo permite cuentas de Google o Microsoft y valida dominios de Gmail/Outlook para asegurar la entrega de avisos. | `AuthLogin.jsx` envía la solicitud a `/auth/login`, mientras `main.py` valida dominios permitidos y persiste la sesión. |
